# Resultados da Pesquisa: Ferramentas Agênticas de IA para Desenvolvimento (Junho 2026)

> Pesquisa realizada em 17 de Junho de 2026, com base no plano definido em `plano_de_pesquisa_ia_2026.md`.
...
graph TD
    ...
    subgraph HARNESS ["⚙️ Harness — O que envolve a LLM"]
...
**🗄️ Banco Vetorial (Vector Database)**
...
| GPT-4.5 | ⚠️ Aposentadoria em 27/Jun/2026 | Ainda disponível temporariamente. |
| o3 | ⚠️ Aposentadoria em 26/Ago/2026 | Modelo de raciocínio legado. |

### Anthropic (Claude)
| Modelo | Status | Observações |
| :--- | :--- | :--- |
| **Claude Fable 5** | ⚠️ Lançado em 09/Jun, acesso temporariamente suspenso em 12/Jun | O mais capaz da Anthropic. Saiu do ar para correções. |
...
| **Claude Sonnet 4.6** | ✅ Ativo (GA) | Melhor relação custo-benefício para codificação diária. |
| **Claude Haiku 4.5** | ✅ Ativo (GA) | Velocidade máxima, tarefas simples e alto volume. |

### Google (Gemini)
| Modelo | Status | Observações |
| :--- | :--- | :--- |
| **Gemini 3.5 Flash** | ✅ Ativo (GA desde 19/Mai/2026) | Modelo padrão do Antigravity e do app Gemini. SOTA em velocidade e custo. Até 2M tokens de contexto. |
| **Gemini 3.5 Pro** | ⚠️ Preview limitado (Vertex AI) | Flagship de raciocínio premium. Lançamento geral previsto para Junho 2026. |
...
#### Modelos Suportados
*   **Modelo Principal:** Gemini 3.5 Flash (SOTA, até **2 milhões de tokens** de contexto).
...
*   **Diferencial:** A janela de contexto de 2M tokens do Gemini 3.5 Flash é a maior do mercado, permitindo ingerir repositórios inteiros de código.
...
| :--- | :--- | :--- | :--- | :--- |
| **Antigravity** (Google) | ✅ Uso gratuito da IDE + API Google AI Studio (com limites) | Google AI Pro: $19.99/mês | Google AI Ultra: $99.99/mês (5x cota) | Google AI Ultra Top: $199.99/mês (20x cota) |
...
| **Devin AI** | Pro: $20/mês | Teams: $80/mês + $40/dev | Cotas de sessão + ACUs (Agent Compute Units) para Enterprise. |
...
## Parte D — Comparativo Geral: Antigravity vs. Claude Code vs. Codex vs. Cursor vs. Devin

| Critério | Antigravity | Claude Code | Codex | Cursor | Devin AI |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Interfaces** | CLI + APP + IDE | CLI + APP | CLI + APP | IDE (VS Code fork) | APP autônomo (Sandbox) |
| **Modelos** | Gemini (nativo) + Claude + GPT via API | Claude Opus/Sonnet/Haiku/Fable | GPT-5.5 | Multi-model (Claude, GPT, Gemini) | Multi-model (Claude, GPT, Gemini) |
| **Contexto Máx.** | **2M tokens** (Gemini 3.5 Flash) | **1M tokens** (Fable 5) | ~200K tokens (GPT-5.5) | Depende do modelo escolhido | Depende do modelo escolhido |
| **Orquestração Multi-Agente** | ✅ Nativa (subagentes dinâmicos) | ⚠️ Limitada (subagentes via SDK) | ⚠️ Básica (Goal Management) | ❌ Não nativa | ❌ Agente único autônomo |
| **Melhor Para** | Equipes iniciando jornada agêntica; projetos grandes | Devs seniores que vivem no terminal; DevOps | Generalistas; prototipagem web rápida | Devs solo que querem controle manual fino | Delegar tarefas completas repetitivas |
| **Entrada** | $19.99/mês | $20/mês | $20/mês | $20/mês | $20/mês |
| **Power User** | $199.99/mês | $200/mês | $200/mês | $200/mês | $200/mês |
| **Estabilidade (Jun/2026)** | ✅ Estável | ⚠️ Fable 5 temporariamente suspenso | ⚠️ Erros de capacidade em pico | ✅ Estável | ✅ Estável |
...
| **⚠️ Evitar** | CLIs (Claude Code, Antigravity CLI). Não há benefício prático sem ambiente de desenvolvimento. |
...
| **Modelo sugerido no CLI** | Claude Opus 4.8 (raciocínio arquitetural) ou Claude Sonnet 4.6 (velocidade no dia a dia). |
| **Plano sugerido** | Claude Max 5x ($100/mês) — o volume e a variedade de linguagens deste perfil esgota o plano Pro rapidamente. |
| **⚠️ Limitação** | Antigravity IDE exigiria trocar o IntelliJ pelo fork do VS Code — custo de transição alto demais para o perfil mais sênior. Não recomendado. |
...
| Aspecto | Recomendação |
| :--- | :--- |
| **Principal (dentro da IDE)** | **Antigravity IDE** (substitui diretamente o VS Code, mantendo toda a familiaridade) |
| **Alternativa imediata** | **Cursor Pro** (se preferir não trocar o VS Code ainda) |
| **Por quê** | O Antigravity é um fork do VS Code — a transição é praticamente transparente. Para um front-end React/Vite/TS, o contexto de 2M tokens do Gemini 3.5 Flash permite ingerir o projeto inteiro de uma vez, resolver imports quebrados, gerar componentes completos e rodar o servidor de preview. O Cursor é uma alternativa conservadora igualmente eficaz se a equipe preferir uma migração gradual. |
| **Fluxo recomendado** | Usar o **Manager Surface** do Antigravity para delegar criação de novos componentes React, geração de testes unitários (Vitest/Jest) e análise de performance de bundle (Vite). |
| **Modelo sugerido** | Gemini 3.5 Flash (velocidade + contexto máximo para projetos grandes de front) ou Claude Sonnet 4.6 (para lógica TS mais complexa). |
| **Plano sugerido** | Google AI Pro ($19.99/mês) — cobre o uso diário sem extrapolações. |
| **⚠️ Atenção** | Se optar pelo Cursor, o plano Pro ($20/mês) também é suficiente para uso inicial. |

---

### ⚙️ Dev Back-end (Java · Spring · Banco de Dados) — IntelliJ IDEA Community Edition
...
| **Modelo sugerido** | Claude Sonnet 4.6 (custo-benefício para Java/Spring no dia a dia). |
| **Plano sugerido** | Claude Pro ($20/mês) — suficiente para back-end com tarefas bem delimitadas. |
| **💡 Dica** | Considere recomendar o upgrade para **IntelliJ Ultimate** a longo prazo: o JetBrains AI Assistant nativo oferece uma experiência muito superior à dos plugins de comunidade. |

---

### 🔧 Dev Full-stack 1 e 2 (PHP) — NetBeans

**Contexto:** O NetBeans é o grande desafio da equipe. É a IDE com **menor suporte nativo de IA** do mercado em 2026. Não há nenhum plugin de primeiro partido de Anthropic, OpenAI ou Google disponível para ele.

| Aspecto | Recomendação |
| :--- | :--- |
| **Opção A (Menor Atrito)** | **Claude Code CLI** em terminal paralelo ao NetBeans |
| **Opção B (Dentro do NetBeans)** | Plugin **Jeddict AI Assistant** ou **Anahata (ASI)** (JetBrains Plugin Portal — versão NetBeans) |
| **Opção C (Migração Leve)** | Migrar para **VS Code** + **Antigravity IDE** (mantendo PHP como linguagem) |
| **Por quê (Opção A)** | Com o Claude Code CLI rodando no terminal ao lado do NetBeans, o agente consegue ler e editar os arquivos PHP do projeto, fazer commits Git, rodar testes e sugerir correções. É o caminho mais rápido sem mudar a IDE. |
| **Por quê (Opção B)** | Os plugins Jeddict e Anahata são soluções nativas para NetBeans. O Jeddict oferece completion e refatoração com IA. O Anahata (ASI) é mais avançado, funcionando como um container AGI dentro do JVM do NetBeans, com capacidade de manipulação direta de código. Porém, ambos são de comunidade — suporte e atualização são menos previsíveis. |
| **Por quê (Opção C)** | Para PHP moderno (Laravel, Symfony), o VS Code + Antigravity ou Cursor oferecem uma experiência muito superior. Requer negociar a transição de IDE com os desenvolvedores. |
| **Fluxo recomendado (Opção A)** | Terminal com Claude Code CLI (modo `--auto-edit`) analisando o projeto PHP → Desenvolvedor revisa as mudanças no NetBeans antes de commitar. |
| **Modelo sugerido** | Claude Sonnet 4.6 (velocidade para PHP no dia a dia). |
| **Plano sugerido** | Claude Pro ($20/mês por dev) — um plano por desenvolvedor. |
| **⚠️ Alerta** | A Opção B (plugins de comunidade) é instável e pode ficar desatualizada. A Opção A é mais robusta a curto prazo. A Opção C é a mais eficiente a longo prazo. |
...
| **Dev Sênior** | IntelliJ Ultimate | JetBrains AI (Codex) + Claude Code CLI | Claude Max 5x | ~$100/mês |
| **Dev Front-end** | VS Code | Antigravity IDE | Google AI Pro | ~$20/mês |
| **Dev Back-end** | IntelliJ Community | Plugin Claude Code + Claude Code CLI | Claude Pro | ~$20/mês |
| **Dev Full-stack 1** | NetBeans | Claude Code CLI (Opção A) | Claude Pro | ~$20/mês |
| **Dev Full-stack 2** | NetBeans | Claude Code CLI (Opção A) | Claude Pro | ~$20/mês |
| **TOTAL ESTIMADO** | | | | **~$200/mês** |

> 💡 **Nota:** O custo total de ~$200/mês cobre toda a equipe com ferramentas de nível profissional. O Dev Sênior consome mais por conta da amplitude de linguagens e do uso intenso de raciocínio arquitetural (Claude Max 5x).

---

### 🗺️ Mapa de Ferramentas por Desenvolvedor

```mermaid
graph LR
    subgraph JETBRAINS ["Ecossistema JetBrains (IntelliJ)"]
        S["🥇 Dev Sênior\nIntelliJ Ultimate"] -->|"Dentro da IDE"| JBA["JetBrains AI Assistant\n(Codex / GPT-5.5)"]
        S -->|"Terminal paralelo"| CC1["Claude Code CLI\n(Opus 4.8 para arquitetura)"]
        BE["⚙️ Dev Back-end\nIntelliJ Community"] -->|"Plugin Marketplace"| CCP["Claude Code Plus Plugin\n(Comunidade)"]
        BE -->|"Terminal paralelo"| CC2["Claude Code CLI\n(Sonnet 4.6)"]
    end

    subgraph VSCODE ["Ecossistema VS Code"]
        FE["🎨 Dev Front-end\nVS Code"] -->|"Migração transparente"| AG["Antigravity IDE\n(Gemini 3.5 Flash)"]
    end

    subgraph NETBEANS ["NetBeans (Solução Híbrida)"]
        FS1["🔧 Full-stack 1\nNetBeans"] -->|"Terminal paralelo"| CC3["Claude Code CLI\n(Sonnet 4.6)"]
        FS2["🔧 Full-stack 2\nNetBeans"] -->|"Terminal paralelo"| CC4["Claude Code CLI\n(Sonnet 4.6)"]
    end
